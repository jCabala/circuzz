...
#include "circom.hpp"
#include "calcwit.hpp"
void XOR_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void XOR_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 289;}

uint get_number_of_components() {return 13;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 289;}
...
Fr_mul(&expaux[2],&circuitConstants[0],&signalValues[mySignalStart + 1]); // line circom 49
...
XOR_0_create(mySignalStart+2,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
...
OR_2_create(mySignalStart+7,2+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 2+ctx_index+1;
}
{
std::string new_cmp_name = "comp_3";
GreaterEqThan_5_create(mySignalStart+10,3+ctx_index+1,ctx,new_cmp_name,myId);
...
Mux1_7_create(mySignalStart+272,7+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[5] = 7+ctx_index+1;
}
{
std::string new_cmp_name = "comp_6";
OR_2_create(mySignalStart+280,9+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[6] = 9+ctx_index+1;
}
{
std::string new_cmp_name = "comp_7";
IsZero_8_create(mySignalStart+283,10+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[7] = 10+ctx_index+1;
}
{
std::string new_cmp_name = "comp_8";
NOT_1_create(mySignalStart+286,11+ctx_index+1,ctx,new_cmp_name,myId);
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
XOR_0_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
cmp_index_ref_load = 8;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[8]].signalStart + 0],&circuitConstants[1]); // line circom 36
}}
