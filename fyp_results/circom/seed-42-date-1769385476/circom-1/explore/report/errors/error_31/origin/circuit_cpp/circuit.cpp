...
uint get_total_signal_no() {return 293;}
...
IsEqual_1_create(mySignalStart+8,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "comp_1";
NOT_2_create(mySignalStart+14,2+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 2+ctx_index+1;
}
{
std::string new_cmp_name = "comp_2";
OR_3_create(mySignalStart+278,7+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 7+ctx_index+1;
}
{
std::string new_cmp_name = "comp_3";
XOR_4_create(mySignalStart+281,8+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[3] = 8+ctx_index+1;
}
{
std::string new_cmp_name = "comp_8";
Mux1_6_create(mySignalStart+284,9+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[4] = 9+ctx_index+1;
}
{
std::string new_cmp_name = "comp_10";
LessThan_8_create(mySignalStart+16,3+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[5] = 3+ctx_index+1;
}
{
std::string new_cmp_name = "comp_11";
IsZero_0_create(mySignalStart+273,5+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[6] = 5+ctx_index+1;
}
{
std::string new_cmp_name = "comp_12";
NOT_2_create(mySignalStart+276,6+ctx_index+1,ctx,new_cmp_name,myId);
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 3]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 5]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 0],&circuitConstants[8]); // line circom 34
Fr_sub(&expaux[0],&circuitConstants[7],&expaux[2]); // line circom 34
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
cmp_index_ref_load = 7;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[7]].signalStart + 0],&circuitConstants[1]); // line circom 46
}}
