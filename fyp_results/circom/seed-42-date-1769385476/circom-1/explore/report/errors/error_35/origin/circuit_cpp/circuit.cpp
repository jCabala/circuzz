...
void LessEqThan_6_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void LessEqThan_6_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 551;}

uint get_number_of_components() {return 14;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 551;}
...
GreaterEqThan_2_create(mySignalStart+9,1+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 1+ctx_index+1;
}
{
std::string new_cmp_name = "comp_3";
NOT_3_create(mySignalStart+269,4+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 4+ctx_index+1;
}
{
std::string new_cmp_name = "comp_4";
Mux1_5_create(mySignalStart+271,5+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 5+ctx_index+1;
}
{
std::string new_cmp_name = "comp_5";
Mux1_5_create(mySignalStart+279,7+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[3] = 7+ctx_index+1;
}
{
std::string new_cmp_name = "comp_8";
LessEqThan_6_create(mySignalStart+287,9+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[4] = 9+ctx_index+1;
}
{
std::string new_cmp_name = "comp_9";
IsZero_7_create(mySignalStart+547,12+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[5] = 12+ctx_index+1;
}
{
std::string new_cmp_name = "comp_10";
NOT_3_create(mySignalStart+7,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[6] = 0+ctx_index+1;
}
// load src
// end load src
signalValues[mySignalStart + 0] = 1547794799609807648ull;
// load src
// end load src
signalValues[mySignalStart + 1] = signalValues[mySignalStart + 2];
// load src
// end load src
signalValues[mySignalStart + 3] = 0ull;
// load src
// end load src
signalValues[mySignalStart + 4] = 14727475742939953530ull;
{
uint cmp_index_ref = 0;
// load src
// end load src
...
// load src
// end load src
ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1] = 7761042409611639294ull;
// no need to run sub component
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter > 0);
}
...
// load src
// end load src
ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2] = 1645597081163114969ull;
// no need to run sub component
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter > 0);
}
...
cmp_index_ref_load = 3;
// end load src
signalValues[mySignalStart + 5] = ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0];
// load src
// end load src
signalValues[mySignalStart + 6] = 13162773539536675389ull;
{
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
LessEqThan_6_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
cmp_index_ref_load = 6;
if (!Fr_isTrue(Fr_eq(ctx->signalValues[ctx->componentMemory[mySubcomponents[6]].signalStart + 0],1ull))) std::cout << "Failed assert in template/function " << myTemplateName << " line 43. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(Fr_eq(ctx->signalValues[ctx->componentMemory[mySubcomponents[6]].signalStart + 0],1ull)));
}
{
printf("<@> out0 = ");
}
