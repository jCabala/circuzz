...
uint get_total_signal_no() {return 314;}
...
LessThan_1_create(mySignalStart+18,5+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 5+ctx_index+1;
}
{
std::string new_cmp_name = "comp_3";
OR_2_create(mySignalStart+275,7+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 7+ctx_index+1;
}
{
std::string new_cmp_name = "comp_4";
AND_3_create(mySignalStart+278,8+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 8+ctx_index+1;
}
{
std::string new_cmp_name = "comp_5";
Mux1_5_create(mySignalStart+281,9+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[3] = 9+ctx_index+1;
}
{
std::string new_cmp_name = "comp_6";
Mux1_5_create(mySignalStart+289,11+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[4] = 11+ctx_index+1;
}
{
std::string new_cmp_name = "comp_7";
Mux1_5_create(mySignalStart+297,13+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[5] = 13+ctx_index+1;
}
{
std::string new_cmp_name = "comp_8";
Mux1_5_create(mySignalStart+305,15+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[6] = 15+ctx_index+1;
}
{
std::string new_cmp_name = "comp_11";
IsEqual_7_create(mySignalStart+5,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[7] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "comp_12";
NOT_8_create(mySignalStart+11,2+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[8] = 2+ctx_index+1;
}
{
std::string new_cmp_name = "comp_13";
IsZero_6_create(mySignalStart+13,3+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[9] = 3+ctx_index+1;
}
{
std::string new_cmp_name = "comp_14";
NOT_8_create(mySignalStart+16,4+ctx_index+1,ctx,new_cmp_name,myId);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_neg(&expaux[0],&signalValues[mySignalStart + 0]); // line circom 37
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
cmp_index_ref_load = 10;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[10]].signalStart + 0],&circuitConstants[2]); // line circom 53
}}
