...
uint get_total_signal_no() {return 326;}

uint get_number_of_components() {return 21;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 271;}
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[1]); // line circom 41
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 41
...
IsEqual_1_create(mySignalStart+5,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "comp_2";
AND_2_create(mySignalStart+287,10+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 10+ctx_index+1;
}
{
std::string new_cmp_name = "comp_3";
Mux1_4_create(mySignalStart+290,11+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 11+ctx_index+1;
}
{
std::string new_cmp_name = "comp_4";
Mux1_4_create(mySignalStart+298,13+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[3] = 13+ctx_index+1;
}
{
std::string new_cmp_name = "comp_5";
XOR_5_create(mySignalStart+306,15+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[4] = 15+ctx_index+1;
}
{
std::string new_cmp_name = "comp_6";
Mux1_4_create(mySignalStart+309,16+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[5] = 16+ctx_index+1;
}
{
std::string new_cmp_name = "comp_7";
Mux1_4_create(mySignalStart+317,18+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[6] = 18+ctx_index+1;
}
{
std::string new_cmp_name = "comp_10";
GreaterThan_8_create(mySignalStart+11,2+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[7] = 2+ctx_index+1;
}
{
std::string new_cmp_name = "comp_11";
Mux1_4_create(mySignalStart+271,5+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[8] = 5+ctx_index+1;
}
{
std::string new_cmp_name = "comp_12";
XOR_5_create(mySignalStart+279,7+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[9] = 7+ctx_index+1;
}
{
std::string new_cmp_name = "comp_13";
IsZero_0_create(mySignalStart+282,8+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[10] = 8+ctx_index+1;
}
{
std::string new_cmp_name = "comp_14";
NOT_9_create(mySignalStart+285,9+ctx_index+1,ctx,new_cmp_name,myId);
...
cmp_index_ref_load = 11;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[11]].signalStart + 0],&circuitConstants[1]); // line circom 57
}}
