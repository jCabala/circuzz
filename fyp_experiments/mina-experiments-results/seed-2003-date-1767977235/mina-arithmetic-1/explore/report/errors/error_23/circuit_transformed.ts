  ...
  methods: {
    compute: {
      ...
      async method(in0: Field) {
        ...
        const out1 = Provable.if(Bool(false).and(Bool(false)).not(), out0.add(Field(8903556654279650709457148459860759019034628173702675846442064519656229066924n).neg()), in0.neg().neg().neg().neg()).div(out0).neg().neg();
        Provable.log(out1);
        Bool(false).and(Field(516144393369525645737047763694370905748882575922377131673246459853060806996n).neg().add(Provable.if(Bool(false), out0, out1)).lessThan(Field(18793052654014145293851612758743523177784737310079833315704221572297374575545n).neg().neg())).assertTrue();
