  ...
  methods: {
    compute: {
      ...
      async method(in0: Field, in1: Field) {
        ...
        Provable.log(out1);
        Bool(false).and(Field(10737027894875933773864960588694488439566180340353813305567427424439520026165n).neg().neg().equals(Field(8412713868800408762440864664875180084533101679437973745482655074053907940578n).mul(in0.neg().neg().mul(Field(1)))).not().not()).assertTrue();
