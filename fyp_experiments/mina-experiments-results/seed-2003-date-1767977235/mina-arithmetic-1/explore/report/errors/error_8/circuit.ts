  ...
  methods: {
    compute: {
      ...
      async method(in0: Field) {
        const out0 = Provable.if(Bool(false).not().or(Bool(false).and(Bool(false))), in0.neg().neg(), Field(28461168768760370220138085473466822053091293307034135425164606655956805139973n).mul(in0).neg()).neg();
        Provable.log(out0);
